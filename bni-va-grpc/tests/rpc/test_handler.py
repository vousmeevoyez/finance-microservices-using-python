...
async def test_create_virtual_account(mock_provider):
    ...
    expected_value = {
        "virtual_account": "123312312312312",
        "trx_id": "1231231231"
    }
...
async def test_inquiry_va(mock_provider, setup_va_object):
    expected_value = {'email': '', 'paid_amount': 0, 'trx_amount': 1, 'created_at': '2018-10-26T06:39:27+07:00', 'billing_type': 'j', 'account_no': '9889909918102605', 'description': '', 'status': 'INACTIVE', 'trx_id': '121', 'updated_at': '2018-10-26T06:43:25+07:00', 'phone_number': '', 'paid_at': None, 'name': 'Jennie', 'ref_number': None, 'expired_at': '2017-10-28T06:39:27+07:00'}
...
async def test_update_va(mock_provider, setup_va_object):
    ...
    result = await BNIVaServices("CREDIT",
                                 setup_va_object.account_no).update_va(payload)
    assert result == expected_value
...
async def test_disable_va(mock_provider, setup_va_object):
    ...
    result = await BNIVaServices("CREDIT",
                                 setup_va_object.account_no).disable_va()
