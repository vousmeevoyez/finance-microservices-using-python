...
def generate_email_template(product_type, email_type, encoded_content=None):
    ...
    email_content = {"CONTENT": EMAIL_TEMPLATES[product_type]["CONTENT"][email_type]}
    ...
    email_template = TemplateEngine(
        template_name=EMAIL_TEMPLATES[product_type]["TEMPLATE"][email_type], data=data
    ).render()
    return email_template


def generate_notification_template(product_type, notif_type, encoded_content=None):
    ...
    if encoded_content is not None:
        decoded_content = decode_content(encoded_content)
        parsed_template = parse_to_template(
            string_template=message, data=decoded_content
        )
