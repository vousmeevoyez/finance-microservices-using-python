...
def test_callback_transaction(mock_post, setup_investment_with_transaction):
    ...
    mock_post.return_value.json.return_value = {"status": "SEND_INVESTMENT_SUCCESS"}
...
def test_send_transaction_retry(
    mock_post, mock_celery, setup_investment_with_transaction
):
    investment, loan_request, transactions = setup_investment_with_transaction
    transaction = transactions[0]
